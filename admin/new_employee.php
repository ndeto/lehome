...
if(!empty($_SESSION['member'])){
  ...
  $sql="insert into employees (`name`,`position`,`date`,`author`,unique_id) values ('$title','$position','$date','$author','$unique4')";
  ...
  if($result){
        ...
        if($_FILES['photo']['size'] != 0){
            ...
            if(in_array($file_ext,$expensions)=== false){
               header ("location: add_employee.php?extension=1");
            }

            elseif($file_size > 3000000){
               header ("location: add_employee.php?size=1");
            }

            elseif(empty($errors)==true){
               move_uploaded_file($file_tmp,"../images/uploads/".$photo);
               $sql="update `employees` set photo='$photo' where unique_id='$unique4'";
               $result = mysqli_query($connection,$sql,MYSQLI_STORE_RESULT);
               if($result){
                header ("location: employees.php?employee=1");
            }  else {
  ...
  }else{header('location: add_employee.php?error=1');}
}
?>
